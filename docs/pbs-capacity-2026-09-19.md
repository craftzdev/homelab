...
79% は目標（§6-3 の約 62%）より高い。残っている 3 世代が
**まだ worker の scsi0 を含んでいる**ためで、これらが抜けるまでの一時的な値である。

| 時点 | 見込み |
|---|---|
| 09-20 02:30 のバックアップ後 | 約 88%（+50 GiB）。**この夜が最も窮屈である** |
| 09-21 00:00 の GC 後 | 約 65%。prune で外れた旧世代（worker scsi0 を含む）が回収される |
| 以降 | 約 76% で定常。`keep-daily=5` へ上げられるのはこの後（§6-3） |

⚠️ 09-20 の朝は空きが 10% 程度まで落ちるため `PBSDatastoreFillingUp`（30%）が
発報する。**移行期間の想定内である**が、`PBSDatastoreWillFill` も鳴る場合は
1 晩あたりの増分が見積もり（50 GiB）を超えている。
...
### 6-3. 移行後に `keep-daily=5` へ上げる

`keep-daily=3` は移行措置である。worker の scsi0 を含む古い世代
（〜2026-09-16）が残っている間は 5 世代が入らない。

| | 現行（全ディスク） | worker scsi0 除外後 |
|---|---:|---:|
| 3 世代 | 267.6 GiB | 156.4 GiB |
| 5 世代 | 384.3 GiB | 244.1 GiB |
| 7 世代 | 426.2 GiB | 261.0 GiB |

古い世代が prune され GC が回ったあと（2026-09-23 以降、`df` が 55% 未満）、
次を実行して 5 世代へ戻す。
