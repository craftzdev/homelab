...
### 5-4. 残作業（2026-09-19 時点で未完了）

| 手順 | 状態 |
|---|---|
| §6 手順 1〜2（保持ポリシーと prune） | ✅ 完了 |
| §6 手順 3（GC による回収） | ⏳ **未完了。92.3 GiB が「削除保留」のまま。** §6-2 の手順で猶予を縮めて回す |
| §6 手順 4（worker scsi0 を対象外に） | ⏳ 未実施 |
| §6 手順 6（バックアップジョブの再開） | ⏳ 未実施。**GC で空きが戻るまで再開しないこと** |

**バックアップは 2026-09-17 以降停止したままである**（`enabled 0`）。
手順 3 を終えて `df` が 62% 程度になったことを確認してから手順 4・6 へ進む。
空きが戻らないまま再開すると、02:30 に同じ ENOSPC を繰り返す。
