...
完全には塞がらない。バケットロックで塞げることは 2026-09-16 に確認したが
（[r2-bucket-lock-2026-09-16.md](r2-bucket-lock-2026-09-16.md)）、
PBS が別筐体の砦として既にあるため **2026-09-17 に見送りを決定した。**
判断の理由と再検討の条件は ADR-0012 に記録してある。
...
# 追記16：moshitoku の復元検証（2026-09-16）

## 79. 結論：復元できた。そして同じ失敗を繰り返した

| テーブル | 復元先 | 本番 |
|---|---:|---:|
| `cases` | **17,274** | **17,274** |
| `case_observations` | **26,903** | **26,903** |
| `case_reward_histories` | **17,485** | **17,485** |
| `auth_user` | **1** | **1** |
| `staged_offers` | 88,579 | 88,688 |
| `collection_batches` | 5,932 | 5,939 |
| DB サイズ / テーブル数 | 139 MB / 29 | 139 MB / 29 |

主要テーブルは完全一致。`staged_offers`（+109）と `collection_batches`（+7）の
差はバックアップ取得後に本番へ書き込まれた分で、スナップショットである以上正常。

## 80. ⚠️ 「moshitoku は書き込みがあるから大丈夫」は誤りだった

§78 で「moshitoku は書き込みがあるため WAL が自然に流れており状況は
umami より良いはずだが、確かめていない」と書いた。**確かめたら失敗した。**

```
00000001000000020000002B: WAL not found
```

切り替え直後に取ったバックアップ `r2-verify-b2hn2` は
`beginWal=...002B` を要求するが、それは切り替え**前**の WAL であり
MinIO にしかなかった。R2 へ流れ始めたのは `...002E` 以降である。

書き込みの有無は、**切り替え時点で取ったバックアップには関係しない。**
効くのはその後のバックアップだけである。

> §78 の記述は「推論であって証拠ではない」と自分で断ったうえで書いたが、
> その推論自体が間違っていた。検証して初めて分かった。

## 81. 一般化：保存先を切り替えた直後のバックアップは復元できない

umami と moshitoku の両方で、**切り替え直後に取った最初のバックアップは
復元できなかった。** 書き込み量に関係しない。

原因は単純で、ベースバックアップが要求する WAL は切り替え前に
アーカイブされており、新しい保存先には存在しないためである。

**保存先を切り替えたら、新しい保存先で WAL が流れ始めたことを確認してから
ベースバックアップを取り直し、そこから復元できることを確かめる。**
それまで旧保存先は消せない。

## 82. 検証で作った不要な操作について

moshitoku の endWal 待ちで zsh の文字列比較（`[ "$a" \> "$b" ]`）を誤り、
判定が常に失敗してループが WAL 切り替えを 12 回余分に実行した。
実害は無い（小さな WAL セグメントが R2 へ余分にアーカイブされただけ）が、
不要な操作だった。

また `n_live_tup`（`pg_stat_user_tables`）が復元先で全て 0 だったため
一瞬データ欠損に見えたが、これは統計情報が復元時にリセットされるためで、
実際の `COUNT(*)` では一致していた。**統計ビューで復元の成否を判断しないこと。**

## 83. 現在の到達点

| | バックアップ先 | 保持 | 復元検証 |
|---|---|---|---|
| umami | R2（クラスタ外） | 7d | ✅ 成功 |
| moshitoku | R2（クラスタ外） | 7d | ✅ 成功 |

**ADR-0008 の S5（ストレージの論理破損で DB とバックアップを同時に失う）が、
実際に戻せることまで含めて塞がった。**

旧 MinIO のバケットは、両方で復元が成立したので原理的には削除できる。
ただし削除は別途判断すること（本記録では削除していない）。
