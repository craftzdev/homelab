...
12:52 JST のチャンク atime 読み取りでは、18:15 の回収対象は0 GiB、
翌00:15には104.104 GiBが既定猶予を超える見込みだった。
これは以後のバックアップから再参照されなかった場合の予測であり、回収済み実績ではない。
...
VM 1102 は12:35 JSTに snapshot モードで再取得を開始し、13:07:19 JSTに正常終了した。
71.54 GiBを処理し、234.96 GiB（78%）を再利用。古いバックアップ1件を正常に prune し、
6台すべてに9月20日・21日・22日の3日分の復元点がそろった。
worker の保持バックアップに `scsi0` は残っていない。VM 1200 の手動バックアップは維持。
稼働 VM の停止・再起動は行っていない。

13:08 JSTの標準 GC 再実行も正常終了。参照中243.983 GiB、回収待ち147.039 GiB、
不良チャンク0。安全猶予前なので追加の回収は0 GiB。再取得後の空きは約41 GiB。

新しい `vm/1102/2026-09-22T03:35:28Z` を PBS の snapshot verification で検証し、
約205秒で正常終了した。検査量57,076.92 / 113,736.00 MiB、エラー0件。
VM の実際のリストア試験とは別の、チャンクとバックアップの整合性検証である。
終了時も Kubernetes 6台はすべて Ready、VM 1102 の QEMU PID は作業前後で不変。
