...
- PBS の容量対策が必要。原因の特定は完了した（`docs/pbs-capacity-2026-09-19.md`）。
  ...
  対処は「worker の scsi0 を対象外にする」＋「保持を実測に合わせる」。
  適用手順は同文書 §6。過去バックアップ削除と保持短縮は本対応では未実施。
- バックアップの再開は容量確保と検証後に行う。
  `scripts/reconcile-pbs-kubernetes-backup.sh` の適用で一時停止を上書きしないこと。
