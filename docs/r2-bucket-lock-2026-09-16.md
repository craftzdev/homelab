...
| 実施状況 | **見送り（2026-09-17 決定）。** 技術的には有効だが採らない（§7） |
...
1Password にある R2 のアイテム 2 つは、どちらも `ACCESS_KEY_ID` /
`SECRET_ACCESS_KEY` だけを持つ Object Read & Write の資格情報で、
§1 のとおりロックには使えない。
...
§1〜§3 のとおり有効だと確認できたが、**適用しない**と決めた。
判断とその理由は [ADR-0012](adr/0012-backup-strategy-revisited.md) の
「バケットロックは入れない」に記録した。要点だけ再掲する。

- PBS が別筐体・別認証で既に最後の砦になっており、ロックはその上の二重化
- 守る対象 231 MB は PBS の VM イメージにも含まれる
- ロック日数と `retentionPolicy` の結び付きが運用上の負債になる
- アカウント全体を取られた場合に守れるかは未検証

**この文書は消さない。** 検証の結論（§1〜§3）と手順（§4）は、
再検討するときにそのまま使える。ADR-0012 に再検討の条件を 3 つ
書いてあり、そのいずれかが起きたら §4 を実行すればよい。

> 未確認のまま放置するのと、確認した上で採らないのは別である。
> 前者は「いつか調べる」が残り続けるが、後者は判断が済んでいる。
